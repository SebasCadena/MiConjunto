...
function Pagos({userId}) {
  ...
  useEffect(() => {
    const cargarCobros = async () => {
      try {
        ...
        const contratosInquilino = contratosData.filter((contrato) => contrato.id_inquilino === userId);
  ...
  return (
      ...
      {authLoading ? (<p>Cargando...</p>) : !user ? ( <p>No hay usuario logueado</p> ) : contratos.length > 0 ? (
        <div>
          {pagos.length > 0 ? (
                ...
                <tbody>
                  {pagos.map((pago) => (
                    <tr key={pago.id_pago} >
                      ...
                      <td className="py-2 px-4 border-b">{
                      contratos.find(contrato => contrato.id === pago.contratoId).nombre_inquilino
                      }</td>
                      <td className="py-2 px-4 border-b">{
                       contratos.find(contrato => contrato.id === pago.contratoId).codigo_apartamento
                      }</td>
                      <td className="py-2 px-4 border-b">${pago.valor_pagado}</td>
                       <td className="py-2 px-4 border-b">
                       {
                        pago.num_factura
                       }</td>
